...
def test_mesas_colores():
    ...
    # Verificar que el archivo de MesasPage tiene los estados correctos
    try:
        with open('ceviche-frontend/src/pages/MesasPage.tsx', 'r', encoding='utf-8') as f:
            content = f.read()

        if 'disponible' in content and 'ocupada' in content and 'limpieza' in content:
            print("✅ MesasPage.tsx tiene los estados correctos")
            return True
        else:
            print("❌ MesasPage.tsx no tiene los estados correctos")
            return False
    except Exception as e:
        print(f"❌ Error al verificar MesasPage.tsx: {e}")
        return False
